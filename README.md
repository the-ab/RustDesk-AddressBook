...
- Deutsche Release-Änderungen für `0.5.22` korrigiert.
- Fehlende Version `0.5.21` in der Release-Historie ergänzt.
- Release-Historie sauber getrennt: `0.5.21` enthält die Versions-/Sicherheitslog-Anpassungen, `0.5.22` die Geräte-/Gruppen-/Status-/Sicherheits-Politur.
...
```text
https://dl.ab-xnet.de/rustdesk-addressbook-update-flat-v0523.txt
```

oder:

```text
https://dl.ab-xnet.de/release-notes-v0523.txt
```
...
```text
RustDesk AddressBook 0.5.23
```

Zusätzlich:

```bash
docker exec -it rustdesk-addressbook grep -n "0.5.23-release-notes-history-fix" /app/app/config.py
```
